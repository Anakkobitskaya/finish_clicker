...
class Main:
    ...
    def write(self):
        self.interface.write(self.screen)

    def start(self):
        """
        Эта функция обрабатывет нажатие мыши и все запускает
        """
    ...
    def try_to_click(self):
        ...
        """
        if self.active_window == "game":
            self.try_to_click_game()
        elif self.active_window == "shop":
            self.try_to_click_shop()
    ...
    def check_that_hit(self, x_pos, y_pos):
        """
        :param x_pos: кортеж из 3 чисел, наименьшее, проверяемое, наибольшее
        :param y_pos: кортеж из 3 чисел, наименьшее, проверяемое, наибольшее
        :return: попали ли на кнопку
        """
        return x_pos[0] < x_pos[1] < x_pos[2] and y_pos[0] < y_pos[1] < y_pos[2]
